function WebOffice2015() {
    ...
    */
    ...
    this.WebOpen = function (mBoolean) {
        ...
        this.WebSetMsgByName("STEPNUM", this.STEPNUM); //加载步骤
    ...
    /*保存文件*/
    this.WebSave = function (type) {
        var httpclient = this.obj.Http; //设置http对象 
        httpclient.Clear();               
        this.WebSetMsgByName("OPTION", "SAVEFILE");
        this.WebSetMsgByName("FILENAME", this.FileName); //加载FileName
        this.WebSetMsgByName("mNodeName", mNodeName);
        this.WebSetMsgByName("mNodePid", mNodePid);
        this.WebSetMsgByName("mDataId", mDataId);
        this.WebSetMsgByName("treeNodeId", treeNodeId);
        this.WebSetMsgByName("treeNodeColor", treeNodeColor);
        this.WebSetMsgByName("userId", userId);
        if(type == '2'){
        	this.WebSetMsgByName("signature", "true");
        	this.WebSetMsgByName("wsId", wsId);
        	this.WebSetMsgByName("stepNum", stepNum);
        }
        
        if(type == '6'){
            this.WebSetMsgByName("wsId", wsId);
            this.WebSetMsgByName("stepNum", stepNum);
            this.WebSetMsgByName("rylx", rylx);
            this.WebSetMsgByName("fydm", fydm);
        }
